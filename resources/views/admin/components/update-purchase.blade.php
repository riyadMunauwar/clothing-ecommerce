<div>
    @if($is_purchase_edit_mode_on)
        <x-ui.edit-modal class="max-w-2xl">
            <div class="bg-white p-5 md:p-10 rounded-md">
                <div class="flex justify-between items-center">
                    <h1 class="font-bold text-xl mb-4">Updated Purchase</h1>

                    <span wire:click.debounce="cancelUpdatePurchase" class="cursor-pointer">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="col-span-2 flex items-center justify-end">
                        <x-button wire:click.debounce="updatePurchase" type="button" class="ml-4">
                            {{ __('Save') }}
                        </x-button>
                        <x-button wire:click.debounce="cancelUpdatePurchase" type="button" class="ml-4">
                            {{ __('Cancel') }}
                        </x-button>
                    </div>
        ...
        <x-ui.loading-spinner wire:loading.flex wire:target="cancelUpdatePurchase, updatePurchase" />
    @endif
</div>