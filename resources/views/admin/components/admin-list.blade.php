<section class="bg-white mx-auto max-w-screen-xl rounded-md p-5">
    ...
    <div class="">
        ...
        <div class="bg-white dark:bg-gray-800 relative z-20 overflow-hidden">
            ...
            <div class="flex items-center justify-start mt-5 mb-10">
                <x-button wire:click.debounce="openAddNewAdminModal" class="ml-4">
                    {{ __('Add New') }}
                </x-button>
            </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">
                                <img class="w-10 h-10 rounded-full object-cover" src="{{ $admin->profile_photo_url ?? '' }}" alt="{{ $admin->name ?? '' }}">
                            </td>
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $admin->name ?? '' }}</th>
                            <td class="px-4 py-3">{{ $admin->email ?? '' }}</td>
                            <td class="px-4 py-3"><span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $admin->getRoleNames()->first() ?? 'None' }}</span></td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button wire:click.debounce="enableAdminEditMode({{ $admin->id }})" type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </button>
                                <button wire:click.debounce="confirmDeleteAdmin({{ $admin->id }})" class="ml-1 text-red-400" type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
