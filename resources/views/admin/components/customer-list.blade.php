<section class="bg-white mx-auto max-w-screen-xl rounded-md p-5">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $customer->email ?? '' }}</th>
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $customer->phone ?? '' }}</th>
                            <td class="px-4 py-3"><span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ count($customer->order ?? []) ?? 'None' }}</span></td>
                            <td class="px-4 py-3"><span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $customer->getRoleNames()->first() ?? 'user' }}</span></td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <button wire:click.debounce="confirmDeleteCustomer({{ $customer->id }})" class="ml-1 text-red-400" type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    <x-ui.loading-spinner wire:loading.flex wire:target="enableCustomerEditMode, confirmDeleteCustomer" />
</section>
