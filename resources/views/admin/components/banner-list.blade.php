<section class="bg-white mx-auto max-w-screen-xl rounded-md p-5">
                ...
                <table class="w-full whitespace-nowrap text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">
                                <img class="block w-full h-20 rounded-md object-contain" src="{{ $banner->imageUrl() }}" alt="{{ $banner->name ?? '' }}">
                            </td>
                            <td class="px-4 py-3">{{ $banner->name ?? '' }}</td>
                            <td class="px-4 py-3">{{ $banner->show_in_page ?? '' }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end">
                                    <button wire:click.debounce="enableBannerEditMode({{ $banner->id }})" type="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                        </svg>
                                    </button>
                                    <button wire:click.debounce="confirmDeleteBanner({{ $banner->id }})" class="ml-1 text-red-400" type="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    <x-ui.loading-spinner wire:loading.flex wire:target="confirmDeleteBanner, enableBannerEditMode" />
</section>


@push('modals')
    <livewire:admin.edit-banner />
@endpush