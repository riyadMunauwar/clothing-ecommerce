        ...
        <x-ui.edit-modal class="max-w-2xl">
            <div class="p-5 md:p-10 bg-white rounded-md">
                ...
                <div>
                            ...
                            <table class="w-full whitespace-nowrap text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b dark:border-gray-700">
                                        <td class="px-4 py-3">
                                            <img class="block w-full h-20 rounded-md object-contain" src="{{ $slide->imageUrl() }}" alt="{{ $slide->name ?? '' }}">
                                        </td>
                                        <td class="px-4 py-3">
                                            <a href="{{ $slide->link }}" class="hover:underline">{{ $slide->slide_link ?? '' }}</a>
                                        </td>
                                        <td class="px-4 py-3">
                    ...
                    @if($caurosel_id)
                        <livewire:admin.create-slide :cauroselId="$caurosel_id"/>
                    @endif
         

                </div>
                <x-ui.loading-spinner wire:loading.flex wire:target="search" />
                <x-ui.loading-spinner wire:loading.flex wire:target="confirmDeleteSlide, enableSlideEditMode, hideSlideList" />
        </x-ui.edit-modal>
    @endif
</div>