<section class="bg-white mx-auto max-w-screen-xl rounded-md p-5">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-1">
                                <span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $product->brand->name ?? 'None' }}</span>
                            </td>
                            <td class="px-4 py-1">
                                @if($product->stock_qty < 10)
                                    <span class="text-red-400">{{ $product->stock_qty }}</span>
                                @else 
                                    <span class="text-green-500">{{ $product->stock_qty }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-1">
                                @if($product->variations_count)
                                    <button wire:click.debounce="showVariationList({{$product->id}})" type="button" class="cursor-pointer">
                                        <span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $product->variations_count ?? '' }}</span>
                                    </button>
                                @else
                                    <span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $product->variations_count ?? '' }}</span>
                                @endif
                            ...
                            <td class="px-4 py-1">
                                ...
                                @if($product->is_grocery)
                                    <span class="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">Grocery</span>
                                @endif

                            </td>
                            <td class="px-4 py-1">
                                <div class="flex items-center gap-1 justify-end">
                                    <button wire:click.debounce="enableAddStockModal({{ $product->id }})" type="button">
    ...
    <x-ui.loading-spinner wire:loading.flex wire:target="search, enableAddStockModal, confirmDeleteProduct, deleteProduct, enableProductEditMode, showVariationList" />
</section>
